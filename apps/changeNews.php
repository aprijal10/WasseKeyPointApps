    ...
    if(!empty($file_tmp1)){
        if(in_array($ekstensi, $ekstensi_diperbolehkan) === true){
            ...
            if($ukuran < 1044070){	            
                ...
                $sql = "UPDATE `_tbl_news` SET `title`='$title', `content`='$content',`picture`='http://localhost/Wasse/news/$nama1', `author`='$_POST[author]', `date_news`='$_POST[date_news]', `modify_by`='".$_SESSION['name']."', `modify_date`='$modify_date' WHERE id='$id'";
                $res = mysql_query($sql);
        ...
        $sql = "UPDATE `_tbl_news` SET `title`='$title', `content`='$content', `author`='$_POST[author]', `date_news`='$_POST[date_news]', `modify_by`='".$_SESSION['name']."', `modify_date`='$modify_date' WHERE id='$id'";
        $res = mysql_query($sql);
